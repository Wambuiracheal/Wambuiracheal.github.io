...
<template>
  <section id="top" class="relative overflow-hidden pt-28 pb-12 sm:pt-32 sm:pb-14">
    ...
    <div class="pointer-events-none absolute inset-0 -z-10">
      ...
      <div class="blob right-[-8rem] top-24 h-80 w-80" style="background: #06b6d4; animation-delay: -6s"></div>
    ...
    <div class="container-page flex flex-row items-center gap-12 2xl:flex-row 2xl:items-stretch 2xl:gap-14">
      <!-- Text column -->
      <div class="2xl:min-w-0 2xl:flex-1">
        ...
        <h1 v-reveal="{ delay: 60 }" class="text-4xl font-extrabold leading-[1.05] sm:text-6xl lg:text-[4.1rem]">
          {{ personal.name }}
        </h1>
        ...
        <p v-reveal="{ delay: 120 }" class="mt-5 text-xl font-medium sm:text-2xl">
          ...
          <span class="text-slate-300">
            <span v-for="(word, i) in roleWords" :key="i" class="mr-[0.3ch] inline-block">{{ word }}</span>
          </span>
        </p>

        <p v-reveal="{ delay: 180 }" class="mt-6 max-w-xl text-base leading-relaxed text-slate-400">
          {{ summary }}
        </p>
      ...
      <div v-reveal="{ delay: 160 }"
        class="relative w-full max-w-[15rem] sm:max-w-[17rem] lg:mx-0 lg:ml-auto lg:-mt-6 lg:max-w-[20rem] 2xl:mt-0 2xl:max-w-none 2xl:flex-1">
        <!-- glow -->
        <div
        ...
        <div
          class="relative aspect-[3/4] overflow-hidden rounded-[1.75rem] border border-white/12 bg-ink-800 shadow-2xl shadow-black/40 2xl:aspect-auto 2xl:h-full">
          ...
          <img :src="photoUrl" alt="Racheal Wambui Njuguna in graduation cap and gown" width="1023" height="1537"
            class="absolute inset-0 h-full w-full object-cover object-top transition-opacity duration-500"
            :class="imgOk ? 'opacity-100' : 'opacity-0'" @load="imgOk = true" @error="imgOk = false" />
          <!-- blend into page -->
          <div
        ...
        <div
          ...
          <span class="leading-tight">
            <span class="block text-xs font-semibold text-slate-100">Best Female Innovator</span>
